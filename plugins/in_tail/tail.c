...
static struct flb_config_map config_map[] = {
    ...
    {
     FLB_CONFIG_MAP_BOOL, "skip_permission_errors", "true",
     0, FLB_TRUE, offsetof(struct flb_tail_config, skip_permission_errors),
     "Skip directories with permission errors instead of failing. When enabled (default), "
     "the plugin will continue processing accessible directories even if some directories "
     "cannot be read due to permission issues. When disabled, any permission error will "
